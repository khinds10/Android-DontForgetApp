...
/**
 * create the basic QuickWidget widget that will use a service call in a separate thread to update the widget elements
 * 
 * @author khinds
 */
public class QuickWidget extends AppWidgetProvider {
	...
	public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
		...
		appWidgetIds = appWidgetManager.getAppWidgetIds(new ComponentName(context, QuickWidget.class));
	...
	public static RemoteViews buildUpdate(Context context, Uri appWidgetUri) {
		...
		RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.quick_widget);

		/** apply an intent to the widget as a whole to launch the MainActivity */
		Intent intent = new Intent(context, ItemsActivity.class);
		PendingIntent pendingIntent = PendingIntent.getActivity(context, 0, intent, 0);
		views.setOnClickPendingIntent(R.id.quickWidgetContainer, pendingIntent);
		return views;
	}
}